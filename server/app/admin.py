...
from server.app.models import AdminUser, Device, DeviceCommand, DeviceConfiguration, DeviceEvent, Release, UpdateCommand
...
def update_device_config(device_id: str, display_name: str = Form(""), interval_minutes: int = Form(...), db: Session = Depends(get_db), admin: AdminUser = Depends(current_admin)):
    device = db.get(Device, device_id)
    if device is None:
        raise HTTPException(status_code=404)
